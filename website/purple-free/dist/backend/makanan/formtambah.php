...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] === 0) {
        ...
        $targetDir = "../../../../assets/img/newmenu/";
        $targetFile = $targetDir . $gambar;

        if (move_uploaded_file($_FILES['gambar']['tmp_name'], $targetFile)) {
            // upload berhasil, lanjut
        } else {
  ...
  <div class="card">
    <div class="card-body">
      ...
      <div class="form-group">
        ...
        <input type="file" name="gambar" class="form-control-file" accept="image/*" onchange="previewImage(this)" required>
        <img id="preview" src="#" alt="Preview" style="max-width: 200px; margin-top: 10px; display: none;">
      </div>
        <button type="submit" class="btn btn-gradient-primary">Simpan</button>
        <a href="../../pages/tables/basic-table-daftarmenu.php" class="btn btn-light">Batal</a>
      </form>
    </div>
  </div>
</div>
...
function uploadGambar(input) {
  ...
  formData.append('gambar_ajax', file);
  ...
  .then(result => {
    if (result.status === 'success') {
      document.getElementById('preview').src = result.filePath;
      document.getElementById('preview').style.display = 'block';
      document.getElementById('gambar').value = result.fileName;
    } else {
